...
private[deploy] class Worker(
  ...
  val pollonAlpha = conf.getDouble("spark.control.pollon.alpha", 1d)
  ...
  val pollon: ControllerPollonAbstract = pollonType match {
    ...
    case "speed" => new ControllerPollonSpeed(cores, Ts, pollonAlpha, conf.getLong("spark.control.avgnominalrate", 1000))
    case "mixed_speed_edf" => new ControllerPollonMixedSpeedEDF(cores, Ts, pollonAlpha, conf.getLong("spark.control.avgnominalrate", 1000))
    // default is proportional
    case _ => new ControllerPollonProportional(cores, Ts, pollonAlpha)
  }
...
//          controllerProxy.proxyEndpoint.send(ResizeOffHeapMemory(offHeapMemory*1000000))
    ...
    case LaunchDriver(driverId, driverDesc) =>
      ...
      val offheapMemory = if (conf.getBoolean("spark.memory.offHeap.enabled", false)) {
        conf.getSizeAsBytes("spark.memory.offHeap.size", 0) / 1000000
      } else {
        0
      }
    ...
    case InitControllerExecutor
      (appId, executorId, stageId, coreMin, coreMax, tasks, deadline, core, deadlineAppTimestamp, nominalRateApp) =>
      ...
      val controllerExecutor = new ControllerExecutor(
        conf, appId, executorId, deadline, coreMin, coreMax, tasks, core, deadlineAppTimestamp, nominalRateApp)
